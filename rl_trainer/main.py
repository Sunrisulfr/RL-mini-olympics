...
sys.path.append(base_dir +"\\olympics_engine")
print(sys.path)
...
def main(args):
    ...
    while episode < args.max_episodes:
        state = env.reset()    #[{'obs':[40,40], "control_player_index": 0}, {'obs':[25,25], "control_player_index": 1}]
        if RENDER:
            env.env_core.render()
        ...
        while True:
            ...
            if type(next_obs_ctrl_agent) == dict:
                next_obs_ctrl_agent = next_obs_ctrl_agent["agent_obs"]
            next_obs_oppo_agent = next_state[1-ctrl_agent_index]['obs']
            if type(next_obs_oppo_agent) == dict:
                next_obs_oppo_agent = next_obs_oppo_agent["agent_obs"]
            # print(next_obs_ctrl_agent)
